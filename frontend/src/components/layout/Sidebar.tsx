...
const navItems = [
  { path: '/call', label: '通话交互', icon: Phone },
  { path: '/agents', label: '客服编排', icon: Users },
  { path: '/rag', label: '知识库管理', icon: FileText },
  { path: '/skills', label: '插件中心', icon: Wrench },
  ...
  return (
    <aside className="w-72 flex flex-col shrink-0 py-6 pl-6 h-screen relative z-10">
      ...
      <div className="mb-10 px-4">
        <NavLink to="/call" className="flex items-center gap-4 group cursor-pointer">
            ...
            >
              {({ isActive }) => (
                <>
                  ...
                  <div className={`relative z-10 transition-all duration-300 ${isActive ? 'scale-110 text-primary-400' : 'group-hover:scale-110 group-hover:text-slate-200'}`}>
                    <Icon size={20} strokeWidth={isActive ? 2.5 : 2} />
                  </div>
                  
                  <span className={`relative z-10 font-semibold tracking-wide transition-colors duration-300 ${isActive ? 'text-white' : 'group-hover:text-slate-200'}`}>
                    {item.label}
                  </span>
